...
// Example usage
try {
  const order = await makeApiRequest('https://api.orders.sdigital2.com/v1/orders/123')
...
# Example usage
try:
    order = make_api_request('https://api.orders.sdigital2.com/v1/orders/123')
    print('Order:', order)
...
try {
  const order = await makeApiRequestWithRetry(
    'https://api.orders.sdigital2.com/v1/orders/123'
  )
  console.log('Order:', order)
} catch (error) {
  console.error('Failed to fetch order:', error)
