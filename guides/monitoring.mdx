...
async function listOrders(options: ListOrdersOptions = {}) {
  ...
  const response = await fetch(`https://api.orders.sdigital2.com/v1/orders?${params}`, {
    headers: {
      'x-api-key': process.env.API_KEY,
    },
  })
...
async function getDailySummary(date: string): Promise<OrderSummary> {
  const response = await fetch(
    `https://api.orders.sdigital2.com/v1/orders/summary?date=${date}`,
    {
      headers: {
        'x-api-key': process.env.API_KEY,
      },
...
def get_daily_summary(date_str: str) -> OrderSummary:
    response = requests.get(
        f'https://api.orders.sdigital2.com/v1/orders/summary',
        headers={
            'Content-Type': 'application/json',
            'x-api-key': os.getenv('API_KEY')
        },
